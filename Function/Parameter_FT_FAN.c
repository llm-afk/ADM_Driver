...
// 控制板参数设置
///////////////////////////////////////////////////////////////////////////////////////
unsigned int MAX_DC_VOL_3_3V        =  930;        //! 0.1vdc 母线电压采集的最大量程 根据实际的硬件电路决定3V和16倍
#define  MAX_PEAK_CUR_3_3V             4688        //! 0.01A // 电流采集的最大量程，由运放增益和采样电阻阻值共同决定 ±20A就填2000
unsigned long MAX_PEAK_CUR_3_3V_2   =  MAX_PEAK_CUR_3_3V;
unsigned long MAX_PEAK_CUR_3_3V_410 =  ((long)MAX_PEAK_CUR_3_3V << 12) / 10;

unsigned int OVER_DC_VOL_POINT    =  380;         //! 0.1vdc // 母线电压过压保护阈值 切断输出
unsigned int OVER_DC_VOL_LIMIT    =  370;         //! 0.1vdc // 母线电压过压控制限制点（非立即保护）降低电流
unsigned int UNDER_DC_VOL_POINT   =  140;         //! 0.1vdc // 母线电压欠压保护阈值 

unsigned int  OVER_DC_VOL_KP      =  800;
unsigned int MaxAllowUdcLimit     =  170;
unsigned int MidAllowUdcLimit     =  155;

#define DEAD_TIME                    5          // 设置死区时间，实际的上下管的死区时间是 DEAD_TIME+20 单位0.01us
...
unsigned int MOTOR_RATED_VOL    =  23;  //! 相电压的有效值，输入实际的母线电压/1.414
unsigned int MOTOR_RATED_CUR    =  280;  //! 0.1A,希望的最大相电流的有效值，注意q轴电流等于相电流的峰值，所以允许的最大q轴电流是MOTOR_RATED_CUR的1.414倍
...
unsigned int D_CUR_KP               =  100;  // !
unsigned int Q_CUR_KI               =  200; // !
unsigned int Q_CUR_KP               =  100;  // !
